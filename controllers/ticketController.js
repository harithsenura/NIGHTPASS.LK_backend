...
// Download Ticket as PDF (Standard Professional Way)
const downloadTicket = async (req, res) => {
  try {
    const { id } = req.params;
    const purchase = await TicketPurchase.findById(id).populate('eventId');
    ...
    // Filename
    const filename = `NightPass-${id.slice(-6).toUpperCase()}.pdf`;
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    doc.pipe(res);

    // 1. Background
    doc.rect(0, 0, 400, 700).fill('#07070a');
    ...
    if (event.image) {
      try {
        ...
        doc.image(imageBuffer, 0, 0, { width: 400, height: 250 });
      } catch (err) {
        console.error("PDF Image fetch error:", err);
        doc.rect(0, 0, 400, 250).fill('#13131a');
      }
    }

    // Gradient Overlay for Image
    doc.save();
    // doc.rect(0, 150, 400, 100).fillColor('black').fillOpacity(0.5).fill(); // Temporarily disabled for simplicity
    doc.restore();

    // 3. Divider Line (Dashed)
    doc.moveTo(20, 250).lineTo(380, 250).dash(5, { space: 5 }).strokeColor('#ffffff').opacity(0.1).stroke();

    // 4. Content Area
    doc.fillColor('#ffffff').opacity(1);
    
    // Event Title
    doc.font('Helvetica-Bold').fontSize(22).text(event.title.toUpperCase(), 30, 280, { width: 340, align: 'left' });
    
    // Details
    doc.fontSize(10).font('Helvetica').opacity(0.6).text('DATE & TIME', 30, 330);
    doc.opacity(1).fontSize(12).text(event.date, 30, 345);

    doc.opacity(0.6).fontSize(10).text('VENUE', 30, 375);
    doc.opacity(1).fontSize(12).text(event.venue || event.location, 30, 390);

    // 5. Ticket Info
    const ticket = purchase.tickets[0]; // For now taking first ticket type
    doc.opacity(0.6).fontSize(10).text('TICKET TIER', 30, 430);
    doc.opacity(1).fontSize(16).font('Helvetica-Bold').fillColor('#10b981').text(ticket.name.toUpperCase(), 30, 445);

    // Unique ID
    const uniqueId = ticket.ticketIds[0];
    doc.fillColor('#ffffff').opacity(0.6).fontSize(10).text('UNIQUE PASS ID', 30, 490);
    doc.opacity(1).fontSize(14).font('Courier-Bold').text(uniqueId, 30, 505);
    ...
    doc.rect(220, 460, 150, 150).fill('#ffffff');
    doc.image(qrBuffer, 220, 460, { width: 150 });

    // 7. Footer Branding
    doc.fillColor('#ffffff').opacity(0.2).fontSize(8).font('Helvetica').text('SECURE DIGITAL PASS GENERATED BY NIGHTPASS.LK', 0, 660, { align: 'center', width: 400 });
    ...
    if (!res.headersSent) {
      res.status(500).json({ message: 'Error generating PDF ticket', error: error.message });
    }
  }
};
...
module.exports = {
  ...
  payhereNotify,
  verifyAvailability,
  downloadTicket // Export the new function
};
